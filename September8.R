# Author: Timothy, Date: Sep 8 2021, Purpose: To perform ANOVA test on sample dataset
...
library(ggplot2)
plot <- ggplot(df, aes(x = poison, y = time, fill = poison)) + geom_boxplot() + geom_jitter(shape = 15, color = "steelblue", position = position_jitter(0.21)) + theme_classic()
print(plot)
